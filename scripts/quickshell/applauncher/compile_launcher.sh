#!/bin/bash
g++ -O3 -std=c++17 app_fetcher.cpp -o app_fetcher
if [ $? -eq 0 ]; then
...
g++ -O3 -std=c++17 tools_fetcher.cpp -o tools_fetcher -lcurl
if [ $? -eq 0 ]; then
...
g++ -O3 -std=c++17 app_launcher_backend.cpp -o app_launcher_backend
if [ $? -eq 0 ]; then
