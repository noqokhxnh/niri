...
if [ -f "focustime/focus_daemon.cpp" ]; then
    echo "-> Compiling focus_daemon..."
    g++ -O3 -std=c++20 -pthread focustime/focus_daemon.cpp -o focustime/focus_daemon -lsqlite3 >/dev/null
fi
if [ -f "focustime/get_stats.cpp" ]; then
    echo "-> Compiling focus_stats..."
    g++ -O3 -std=c++20 focustime/get_stats.cpp -o focustime/get_stats -lsqlite3 >/dev/null
fi

# 10. Compile sys_fetcher
if [ -f "watchers/sys_fetcher.cpp" ]; then
    echo "-> Compiling sys_fetcher..."
    g++ -O3 -std=c++20 watchers/sys_fetcher.cpp -o watchers/sys_fetcher >/dev/null
fi
